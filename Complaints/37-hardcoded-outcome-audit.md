...
`hardcoded_outcome` beyond the two above. This audit should be
...
no production function reads; none is a `hardcoded_outcome`
...
`hardcoded_outcome` - each is recorded so a reviewer can overrule
this call:
...
`hardcoded_outcome` rather than `engineering_estimate`:
...
`temporary_heuristic`-shaped, not `hardcoded_outcome`-shaped.
